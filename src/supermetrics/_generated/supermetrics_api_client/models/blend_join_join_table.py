...
T = TypeVar("T", bound="BlendJoinJoinTable")


@_attrs_define
class BlendJoinJoinTable:
    """Data source to join with the primary table. Provide `blend_data_source_key` when creating, `blend_data_source_id`
    when updating with an existing data source.

        Attributes:
            blend_data_source_id (int | None | Unset): Internal ID of the data source to join. Example: 146715.
            blend_data_source_key (None | str | Unset): Key of the data source to join (for new data sources). Example:
                abcd1234.
    """
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        blend_join_join_table = cls(
            blend_data_source_id=blend_data_source_id,
            blend_data_source_key=blend_data_source_key,
        )

        blend_join_join_table.additional_properties = d
        return blend_join_join_table
